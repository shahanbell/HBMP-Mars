...
Page({
  ...
  scrollFn(e) { 
    ...
    getSpareListInfo: function(res){
      let _this = this;
      var restUrl = app.globalData.restAdd + '/Hanbell-JRS/api/shbeam/sparemanagement/getSpareStockList';
      ...
      wx.request({
        ...
        success: function (res) {
          ...
          for(var i = 0;i < spareListDta.length; i++){
            ...
            newItem.uPrice = spareListDta[i].equipmentSpareStockList[0].uprice;
            ...
            newItem.itemStockDataList = spareListDta[i].equipmentSpareStockList;
    ...
    if (canSubmit) {
      Dialog.confirm({
        ...
        .then(() => {
          ...
          wx.request({
            url: app.globalData.restAdd + '/Hanbell-JRS/api/shbeam/sparemanagement/startSpareDelivery' + '?' + app.globalData.restAuth,
  ...
  spareQtyChange:function(index,newQty){
    ...
    {
      stockQtyTemp = stockQtyTemp + itemStockList[i].qty;
      if(newQty <= stockQtyTemp){
        stockMaxIndex = i;
        let spareItemTemp = JSON.parse(JSON.stringify(itemStockList[i]));
        spareItemTemp.qty = newQty - (stockQtyTemp - itemStockList[i].qty);
        spareItem.itemStockCurrentList.push(spareItemTemp);
        totalPrice = totalPrice + spareItemTemp.uprice * spareItemTemp.qty;
        break;
      }
      else{
        let spareItemTemp = JSON.parse(JSON.stringify(itemStockList[i]));
  ...
  checkFormDtaBeforeSubmit: function(){
    ...
    if(this.data.repairAreaObj.repairAreaValue == '-1'){
      Dialog.alert({
      title: '系统消息',
      message: "请选择所属厂区!",
      zIndex:1000,
