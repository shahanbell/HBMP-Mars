...
Page({
  ...
  bindBudgetacc(e) {
    let that = this
    wx.navigateTo({
      url: '../budgetacc/budgetacc?centerid='+this.data.centerid,
      events: {
  ...
  formSubmit(e) {
    ...
    //   // idx：当前index
    //   // array: Array
    //   if (value.notempty.key) {
    //     if (checkobj[value.key] == "") {
    //       wx.showModal({
    //         title: '系统提示',
    //         content: value.notempty.msg,
    //         showCancel: false
    //       })
    //       return false;
    //     } else {
    //       if (value.isnumber.key) {
    //         if (isNaN(checkobj[value.key])) {
    //           console.log(value.isnumber.msg)
