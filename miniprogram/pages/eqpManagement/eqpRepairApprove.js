...
upload: function(e) {
    ...
    wx.request({
      ...
      success: function (res) {
        ...
        if(res.data.length > 0){
          ...
          var imagePathArray = [];
          for(var i = 0;i < dataLen;i++){
            imagePathArray = imagePathArray.concat([app.globalData.restAdd + "/Hanbell-EAM/resources/app/res/" + res.data[2][i].filename]);
          }
  ...
  onDoApproveBtnClick: function(e){
    ...
    wx.navigateTo({
      url: '../eqpManagement/eqpRepairDoApproveCheck?eqpInfo=' + eqpInfoObj + '&docFormid=' + that.data.docFormid + '&docId=' + that.data.docId + '&rStatus=' + that.data.rStatus
    });
  },
